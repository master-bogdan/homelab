...
```bash
ENV=staging # or prod
kubectl -n $NS exec -i deploy/vault -- vault policy write homelab-$ENV - <<EOF
path "kv/data/$ENV/*" {
  capabilities = ["read"]
}
EOF

kubectl -n $NS exec -i deploy/vault -- vault write auth/kubernetes/role/homelab-$ENV \
...
For `ENV=staging`, use role name `homelab-staging` and a policy scoped to `kv/staging/*`.

### 5) Put secrets into Vault

Fill in `.env` at repo root, then write values into Vault under `kv/<env>/*` (e.g., `kv/staging/*`):

```bash
# Example (grafana)
kubectl -n $NS exec -i deploy/vault -- vault kv put kv/staging/grafana \
...
These are the exact keys ESO reads from Vault (KV v2), where `<env>` is `prod` or `staging`:
