...
```bash
kubectl -n secrets exec -it deploy/vault -- vault operator init
kubectl -n secrets exec -it deploy/vault -- vault operator unseal
...
```bash
kubectl -n secrets exec -it deploy/vault -- vault secrets enable -path=kv kv-v2
kubectl -n secrets exec -it deploy/vault -- vault auth enable kubernetes
```
...
kubectl -n secrets create token external-secrets > /tmp/es.jwt
kubectl config view --raw --minify --output 'jsonpath={.clusters[0].cluster.certificate-authority-data}' \
  | base64 -d > /tmp/ca.crt

kubectl -n secrets exec -i deploy/vault -- vault write auth/kubernetes/config \
...
```bash
kubectl -n secrets exec -i deploy/vault -- vault policy write homelab-prod - <<'EOF'
path "kv/data/prod/*" {
  capabilities = ["read"]
}
EOF

kubectl -n secrets exec -i deploy/vault -- vault write auth/kubernetes/role/homelab-prod \
...
```bash
# Example (grafana)
kubectl -n secrets exec -i deploy/vault -- vault kv put kv/prod/grafana \
